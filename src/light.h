...
__device__ inline int sample_light(int num_lights, RNGf& rng) {
    return floor(random_double(rng) * num_lights);
}
...
__device__ inline PointAndNormal sample_on_light_DiffuseArea(const DiffuseAreaLight &l, const ShapeArr& shapes, const TriangleMeshArr &meshes, const Vector3 &ref_pos, RNGf& rng)
{
    Shape s = shapes[l.shape_id];
    return sample_on_shape(meshes, s, ref_pos, rng);
}

__device__ inline PointAndNormal sample_on_light(const ShapeArr &shapes, const TriangleMeshArr &meshes, const Light& l, const Vector3 &ref_pos, RNGf& rng) {
